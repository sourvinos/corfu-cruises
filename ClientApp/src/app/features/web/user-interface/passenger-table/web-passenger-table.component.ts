...
@Component({
    selector: 'web-passenger-table',
    templateUrl: './web-passenger-table.component.html',
    styleUrls: ['./web-passenger-table.component.css']
})

export class WebPassengerTableComponent {
    ...
    public onSendRowToService(): void {
        ...
        if (document.getElementsByClassName('mat-dialog-container').length === 0) {
            this.interactionService.sendReservation(this.records[this.currentRow - 1])
        } else {
    ...
    private focusOnHiddenInput(): void {
        setTimeout(() => {
            document.getElementById('passenger-table-input-' + this.randomTableId).focus()
        }, 300)
    }
    ...
    private initVariables(): void {
        this.table = document.getElementById('passenger-table-' + this.randomTableId)
        this.tableContainer = this.table.parentNode.parentNode
        this.rowHeight = 41
        this.rowCount = this.table.rows.length - 1
        document.getElementById('passenger-table-input-' + this.randomTableId).style.zIndex = '-1'
        document.getElementById('passenger-table-input-' + this.randomTableId).style.position = 'absolute'
    }
