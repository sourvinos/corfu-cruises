...
export class BookingFormComponent {
    ...
    public onDelete(): void {
        this.dialogService.open('warningColor', this.messageSnackbarService.askConfirmationToDelete(), ['abort', 'ok']).subscribe(response => {
            if (response) {
                this.bookingService.delete(this.form.value.rsvId).subscribe(() => {
                    this.showSnackbar(this.messageSnackbarService.recordDeleted(), 'info')
                    this.onGoBack()
                    this.interactionService.removeTableRow(this.getRowIndex(this.form.value.rsvId))
                    this.resetForm()
    ...
    public onSave(): void {
        if (this.form.value.rsvId === 0 || this.form.value.rsvId === null) {
            ...
            this.bookingService.update(this.form.value.rsvId, this.form.value).subscribe(() => {
                this.resetForm()
    ...
    private flattenDetails(passengers: RsvPassenger[]): any {
        const passengersFlat = []
        passengers.forEach(passenger => {
            const passengerFlat = {
                "id": passenger.id,
                "rsvId": passenger.rsvId,
