context('Registrars', () => {
    ...
    describe('Validate', () => {
        ...
        it('Choose not to abort when the back icon is clicked', () => {
            ...
            cy.url().should('eq', Cypress.config().baseUrl + '/shipRegistrars/new')
        })

        it('Choose to abort when the back icon is clicked', () => {
            cy.intercept('GET', Cypress.config().baseUrl + '/api/shipRegistrars', { fixture:'ships/registrars/registrars.json' }).as('getRegistrars')
            cy.get('[data-cy=goBack]').click()
            cy.get('.mat-dialog-container')
