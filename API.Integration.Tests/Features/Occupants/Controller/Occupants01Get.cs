...
namespace API.IntegrationTests.Occupants {
    ...
    [Collection("Sequence")]
    public class Occupants01Get : IClassFixture<AppSettingsFixture> {
        ...
        public Occupants01Get(AppSettingsFixture appsettings) {
        ...
        public async Task Active_Admins_Can_List() {
            ...
            // act
            var actionResponse = await _httpClient.SendAsync(request);
            var records = JsonSerializer.Deserialize<List<OccupantListResource>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            // assert
