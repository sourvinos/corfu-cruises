...
namespace API.IntegrationTests.PickupPoints {
    ...
    public class PickupPoints05Put : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Active_Admins_Can_Not_Update_When_Invalid(TestPickupPoint record) {
            // arrange
        ...
        [Theory]
        [ClassData(typeof(UpdateValidPickupPoint))]
        public async Task Active_Admins_Can_Update_When_Valid(TestPickupPoint record) {
            // arrange
            var loginResponse = await Helpers.Login(_httpClient, Helpers.CreateLoginCredentials("john", "ec11fc8c16da"));
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
