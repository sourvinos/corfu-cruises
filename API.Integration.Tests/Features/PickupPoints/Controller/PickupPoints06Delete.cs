...
namespace API.IntegrationTests.PickupPoints {
    ...
    public class PickupPoints06Delete : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Not_Found_When_Not_Exists() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            var request = Helpers.CreateRequest(_baseUrl, "/pickupPoints/999");
            // act
        ...
        public async Task Simple_Users_Can_Not_Delete() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/pickupPoints/1");
            // assert
        ...
        public async Task Admins_Can_Not_Delete_In_Use() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/pickupPoints/8");
            // assert
        ...
        public async Task Admins_Can_Delete_Not_In_Use() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/pickupPoints/9");
            // assert
