...
namespace API.IntegrationTests.Genders {
    ...
    public class Genders06Delete : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Unauthorized_Not_Logged_In() {
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/genders/1");
            // assert
        ...
        public async Task Unauthorized_Inactive_Admins() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/genders/4");
            // assert
            Assert.Equal(HttpStatusCode.Unauthorized, actionResponse.StatusCode);
        }
        ...
        public async Task Not_Found_When_Not_Exists() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            var request = Helpers.CreateRequest(_baseUrl, "/genders/99");
            // act
        ...
        public async Task Simple_Users_Can_Not_Delete() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/genders/3");
            // assert
