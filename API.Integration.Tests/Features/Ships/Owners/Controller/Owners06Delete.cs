...
namespace API.IntegrationTests.Ships.Owners {

    public class Owners06Delete : IClassFixture<AppSettingsFixture> {
        ...
        public Owners06Delete(AppSettingsFixture appsettings) {
        ...
        public async Task Unauthorized_Not_Logged_In() {
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/shipOwners/1");
            // assert
        ...
        public async Task Unauthorized_Inactive_Admins() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/shipOwners/4");
            // assert
            Assert.Equal(HttpStatusCode.Unauthorized, actionResponse.StatusCode);
        }
        ...
        public async Task Not_Found_When_Not_Exists() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            var request = Helpers.CreateRequest(_baseUrl, "/shipOwners/99");
            // act
        ...
        public async Task Admins_Can_Delete_Not_In_Use() {
            ...
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            // act
            var actionResponse = await _httpClient.DeleteAsync(_baseUrl + "/shipOwners/2");
            // assert
