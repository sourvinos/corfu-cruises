...
namespace API.IntegrationTests.Ports {
    ...
    public class Ports01Get : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Active_Admins_Can_List() {
            ...
            // act
            var actionResponse = await _httpClient.SendAsync(request);
            var records = JsonSerializer.Deserialize<List<PortListResource>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            // assert
