...
namespace API.IntegrationTests.ShipRoutes {
    ...
    public class ShipRoutes01Get : IClassFixture<AppSettingsFixture> {
        ...
        public async Task Admins_Can_List() {
            ...
            // act
            var actionResponse = await _httpClient.SendAsync(request);
            var records = JsonSerializer.Deserialize<List<ShipRouteListResource>>(await actionResponse.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
            // assert
