...
namespace API.IntegrationTests.Routes {
    ...
    public class Routes04Post : IClassFixture<AppSettingsFixture> {
        ...
        [Theory]
        [ClassData(typeof(NewMinimalRoute))]
        ...
        [Theory]
        [ClassData(typeof(NewMinimalRoute))]
        public async Task Simple_Users_Can_Not_Create(TestRoute record) {
            // arrange
        ...
        [Theory]
        [ClassData(typeof(AdminsCantCreateWhenInvalid))]
        public async Task Admins_Can_Not_Create_When_Invalid(TestRoute record) {
            // arrange
        ...
        [Theory]
        [ClassData(typeof(NewMinimalRoute))]
        public async Task Admins_Can_Create_When_Valid(TestRoute record) {
            // arrange
            var loginResponse = await Helpers.Login(_httpClient, Helpers.CreateLoginCredentials("john", "ec11fc8c16da"));
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(JwtBearerDefaults.AuthenticationScheme, loginResponse.Token);
            record.UserId = loginResponse.UserId;
