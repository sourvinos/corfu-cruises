...
Cypress.Commands.add('gotoEmptyShipRouteForm', () => {
    ...
    cy.url().should('eq', Cypress.config().homeUrl + '/shipRoutes/new')
})

Cypress.Commands.add('readShipRouteRecord', () => {
    cy.intercept('GET', Cypress.config().apiUrl + '/shipRoutes/1', { fixture:'ships/routes/route.json' }).as('getShipRoute')
